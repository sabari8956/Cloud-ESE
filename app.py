...
def get_next_attendance_number():
    """Generate unique attendance number using timestamp"""
    from datetime import datetime
    # Use timestamp-based numbering to avoid need for listing bucket contents
...
def upload_attendance():
    ...
    try:
        ...
        # Get unique attendance identifier (timestamp-based)
        attendance_id = get_next_attendance_number()
        new_filename = f'attendence-{attendance_id}.csv'

        # Upload to S3 using direct HTTP PUT request (works for public-write buckets)
        ...
        if upload_response.status_code in [200, 204]:
            ...
            # If direct upload fails, it means the bucket requires authentication
            return jsonify({
                'success': False,
                'error': f'Upload failed. Bucket may require AWS credentials. Status: {upload_response.status_code}'
            }), 500
